"""End-to-end feature test for the get_home_view MCP tool.
...
async def _call_get_home_view_tool(client: httpx.AsyncClient, authorization: str | None):
    headers = {
    ...
    return await client.post(
        ...
        json={
            ...
            "id": 2,
            "method": "tools/call",
            "params": {"name": "get_home_view", "arguments": {}},
        },
    )
...
async def _mcp_client():
    """See `test_mcp_set_goal_progress.py::_mcp_client` for why a fresh
    ...
    test_mcp.add_tool(
        mcp_server.get_home_view,
        name="get_home_view",
        description=mcp_server.mcp._tool_manager._tools["get_home_view"].description,
    )
...
@pytest.mark.asyncio
async def test_get_home_view_through_live_mcp_transport_returns_html_resource_with_greeting(
    monkeypatch,
):
    user_row = ("Sam", EMAIL)
    ...
    async with _mcp_client() as client:
        response = await _call_get_home_view_tool(client, f"Bearer {token}")
...
@pytest.mark.asyncio
async def test_get_home_view_through_live_mcp_transport_rejects_missing_jwt_before_db_call(
    monkeypatch,
):
    ...
    async with _mcp_client() as client:
        response = await _call_get_home_view_tool(client, None)
...
@pytest.mark.asyncio
async def test_get_home_view_through_live_mcp_transport_rejects_expired_jwt_before_db_call(
    monkeypatch,
):
    fake_connection, _ = _patch_db(monkeypatch, [])
    _patch_user_upsert(monkeypatch)
