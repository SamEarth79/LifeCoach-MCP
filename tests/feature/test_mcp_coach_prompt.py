"""End-to-end feature test for the `coach` MCP prompt.

Drives the actual mounted ASGI app through the real MCP streamable-HTTP
wire protocol (initialize -> notifications/initialized -> prompts/get), the
same way `test_mcp_get_home_view.py` exercises `get_home_view` - this
proves the Authorization header genuinely reaches the prompt handler via
the live HTTP request the MCP SDK constructs from the ASGI scope, and that
the prompt is invoked explicitly via `prompts/get`, not auto-run at
session start.
"""
...
async def _call_coach_prompt(client: httpx.AsyncClient, authorization: str | None):
    headers = {
...
async def _mcp_client():
    ...
    test_mcp.add_prompt(mcp_server.mcp._prompt_manager._prompts["coach"])
...
@pytest.mark.asyncio
async def test_coach_prompt_through_live_mcp_transport_returns_home_view_grounded_message(
    monkeypatch,
):
    user_row = ("Sam", EMAIL)
    ...
    async with _mcp_client() as client:
        response = await _call_coach_prompt(client, f"Bearer {token}")
...
@pytest.mark.asyncio
async def test_coach_prompt_through_live_mcp_transport_rejects_missing_jwt_before_db_call(
    monkeypatch,
):
    ...
    async with _mcp_client() as client:
        response = await _call_coach_prompt(client, None)
...
@pytest.mark.asyncio
async def test_coach_prompt_through_live_mcp_transport_rejects_expired_jwt_before_db_call(
    monkeypatch,
):
    fake_connection, _ = _patch_db(monkeypatch, [])
    _patch_user_upsert(monkeypatch)
