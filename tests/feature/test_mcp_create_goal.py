"""End-to-end feature test for the create_goal MCP tool.
...
same way `test_mcp_delete_goal.py` exercises `delete_goal` — this proves
...
tool's refreshed-home-view `dict` response serializes as `TextContent`
over the wire.
"""
...
async def _call_create_goal_tool(client: httpx.AsyncClient, authorization: str | None, arguments: dict):
    headers = {
    ...
    return await client.post(
        ...
        json={
            ...
            "id": 2,
            "method": "tools/call",
            "params": {"name": "create_goal", "arguments": arguments},
        },
    )
...
async def _mcp_client():
    ...
    test_mcp.add_tool(
        mcp_server.create_goal,
        name="create_goal",
        description=mcp_server.mcp._tool_manager._tools["create_goal"].description,
    )
...
@pytest.mark.asyncio
async def test_create_goal_through_live_mcp_transport_returns_refreshed_home_view_with_new_goal(
    monkeypatch,
):
    refresh_responses = [
        ("Sam", EMAIL),
        [(GOAL_ID, "Get 5000 LinkedIn followers", None)],
        [],
    ]
    fake_connection, captured = _patch_db(monkeypatch, refresh_responses)
    _patch_user_upsert(monkeypatch)
    token = _make_token()

    async with _mcp_client() as client:
        response = await _call_create_goal_tool(
            client,
            f"Bearer {token}",
            {"title": "Get 5000 LinkedIn followers", "description": "Grow my personal brand"},
        )
    ...
    assert "Get 5000 LinkedIn followers" in body_text
    ...
    assert insert_params == (USER_ID, "Get 5000 LinkedIn followers", "Grow my personal brand")


@pytest.mark.asyncio
async def test_create_goal_through_live_mcp_transport_rejects_missing_jwt_before_db_call(monkeypatch):
    fake_connection, _ = _patch_db(monkeypatch, [])
    _patch_user_upsert(monkeypatch)

    async with _mcp_client() as client:
        response = await _call_create_goal_tool(client, None, {"title": "Run a 5k"})
...
@pytest.mark.asyncio
async def test_create_goal_through_live_mcp_transport_rejects_expired_jwt_before_db_call(monkeypatch):
    fake_connection, _ = _patch_db(monkeypatch, [])
    _patch_user_upsert(monkeypatch)
    token = _make_token(exp=0)

    async with _mcp_client() as client:
        response = await _call_create_goal_tool(client, f"Bearer {token}", {"title": "Run a 5k"})
...
@pytest.mark.asyncio
async def test_create_goal_through_live_mcp_transport_rejects_blank_title_before_db_call(monkeypatch):
    fake_connection, _ = _patch_db(monkeypatch, [])
    _patch_user_upsert(monkeypatch)
    token = _make_token()

    async with _mcp_client() as client:
        response = await _call_create_goal_tool(client, f"Bearer {token}", {"title": "   "})
