...
_COACH_INSTRUCTIONS = (
    ...
    "Whenever you create a goal, create it first with no todos. After "
    "the goal is created, discuss with the user what concrete next steps "
    "make sense before adding any todos via create_todo. Once a "
...
@mcp.tool(
    description=(
        ...
        "timeline, or why it matters, if they shared that. Do not include "
        "any todos in this call — create the goal first, then discuss "
        "next steps with the user before adding todos via create_todo. "
        "On success, returns a refreshed home screen UI resource "
        "reflecting the new goal."
    )
)
